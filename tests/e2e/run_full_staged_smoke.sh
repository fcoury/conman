...
request_assert_200 POST "/api/repos/${APP_ID}/members" "$TOKEN_ADMIN" "{\"user_id\":\"${REVIEWER_ID}\",\"role\":\"reviewer\"}" >/dev/null

echo "[4/11] Setting environments + creating workspace"
file=$(request_assert_200 PATCH "/api/repos/${APP_ID}/environments" "$TOKEN_ADMIN" "{\"environments\":[{\"name\":\"prod\",\"position\":1,\"is_canonical\":true,\"runtime_profile_id\":null}]}")
ENV_ID=$(jq -r ".data[0].id" "$file")
rm -f "$file"

file=$(request_assert_200 GET "/api/repos/${APP_ID}/workspaces" "$TOKEN_ADMIN")
WORKSPACE_ID=$(jq -r ".data[0].id" "$file")
rm -f "$file"

request_assert_200 PATCH "/api/repos/${APP_ID}/settings" "$TOKEN_ADMIN" "{\"file_size_limit_bytes\":128}" >/dev/null
request_expect_status 403 PUT "/api/repos/${APP_ID}/workspaces/${WORKSPACE_ID}/files" "$TOKEN_ADMIN" "{\"path\":\".git/config\",\"content\":\"YmxvY2tlZA==\",\"message\":\"blocked-path-check\"}"
LARGE_CONTENT_B64=$(python3 - <<'PY'
import base64
print(base64.b64encode(b"a" * 512).decode())
PY
)
request_expect_status 400 PUT "/api/repos/${APP_ID}/workspaces/${WORKSPACE_ID}/files" "$TOKEN_ADMIN" "{\"path\":\"config/too-big.txt\",\"content\":\"${LARGE_CONTENT_B64}\",\"message\":\"size-guardrail-check\"}"

echo "[5/11] Writing workspace file"
CONTENT_B64=$(printf 'feature: staged-%s\n' "$TS" | base64 | tr -d '\n')
file=$(request_assert_200 PUT "/api/repos/${APP_ID}/workspaces/${WORKSPACE_ID}/files" "$TOKEN_ADMIN" "{\"path\":\"config/app.yaml\",\"content\":\"${CONTENT_B64}\",\"message\":\"update config\"}")
WRITE_SHA=$(jq -r ".data.commit_sha" "$file")
rm -f "$file"

echo "[6/11] Changeset create/submit/approve/queue"
file=$(request_assert_200 POST "/api/repos/${APP_ID}/changesets" "$TOKEN_ADMIN" "{\"workspace_id\":\"${WORKSPACE_ID}\",\"title\":\"Staged changeset ${TS}\",\"description\":\"e2e\"}")
CHANGESET_ID=$(jq -r ".data.id" "$file")
rm -f "$file"

file=$(request_assert_200 POST "/api/repos/${APP_ID}/changesets/${CHANGESET_ID}/submit" "$TOKEN_ADMIN" "{}")
SUBMIT_JOB_ID=$(jq -r ".data.job.id" "$file")
rm -f "$file"
wait_job "$APP_ID" "$TOKEN_ADMIN" "$SUBMIT_JOB_ID" "msuite_submit"

request_assert_200 POST "/api/repos/${APP_ID}/changesets/${CHANGESET_ID}/review" "$TOKEN_REVIEWER" "{\"action\":\"approve\"}" >/dev/null
request_assert_200 POST "/api/repos/${APP_ID}/changesets/${CHANGESET_ID}/queue" "$TOKEN_ADMIN" >/dev/null

echo "[7/11] Release draft/assemble/publish"
file=$(request_assert_200 POST "/api/repos/${APP_ID}/releases" "$TOKEN_ADMIN")
RELEASE_ID=$(jq -r ".data.id" "$file")
RELEASE_TAG=$(jq -r ".data.tag" "$file")
rm -f "$file"

request_assert_200 POST "/api/repos/${APP_ID}/releases/${RELEASE_ID}/changesets" "$TOKEN_ADMIN" "{\"changeset_ids\":[\"${CHANGESET_ID}\"]}" >/dev/null
file=$(request_assert_200 POST "/api/repos/${APP_ID}/releases/${RELEASE_ID}/assemble" "$TOKEN_ADMIN")
ASSEMBLE_JOB_ID=$(jq -r ".data.job.id" "$file")
rm -f "$file"
wait_job "$APP_ID" "$TOKEN_ADMIN" "$ASSEMBLE_JOB_ID" "release_assemble"

r=$(request POST "/api/repos/${APP_ID}/releases/${RELEASE_ID}/publish" "$TOKEN_ADMIN")
status=${r%%|*}; file=${r#*|}
if [[ "$status" == "409" ]]; then
  rm -f "$file"
  MERGE_JOB_ID=$(latest_job_id "$APP_ID" "$TOKEN_ADMIN" "msuite_merge" "${RELEASE_ID}")
  wait_job "$APP_ID" "$TOKEN_ADMIN" "$MERGE_JOB_ID" "msuite_merge"
  file=$(request_assert_200 POST "/api/repos/${APP_ID}/releases/${RELEASE_ID}/publish" "$TOKEN_ADMIN")
...
r=$(request POST "/api/repos/${APP_ID}/environments/${ENV_ID}/deploy" "$TOKEN_ADMIN" "{\"release_id\":\"${RELEASE_ID}\"}")
status=${r%%|*}; file=${r#*|}
if [[ "$status" != "409" ]]; then
  cat "$file" >&2
  rm -f "$file"
...
rm -f "$file"
DRIFT_JOB_ID=$(latest_job_id "$APP_ID" "$TOKEN_ADMIN" "runtime_profile_drift_check" "${ENV_ID}")
wait_job "$APP_ID" "$TOKEN_ADMIN" "$DRIFT_JOB_ID" "drift_check"
...
rm -f "$file"
DEPLOY_GATE_JOB_ID=$(latest_job_id "$APP_ID" "$TOKEN_ADMIN" "msuite_deploy" "${ENV_ID}:${RELEASE_ID}")
wait_job "$APP_ID" "$TOKEN_ADMIN" "$DEPLOY_GATE_JOB_ID" "msuite_deploy"

file=$(request_assert_200 POST "/api/repos/${APP_ID}/environments/${ENV_ID}/deploy" "$TOKEN_ADMIN" "{\"release_id\":\"${RELEASE_ID}\"}")
DEPLOYMENT_JOB_ID=$(jq -r ".data.job.id" "$file")
DEPLOYMENT_ID=$(jq -r ".data.deployment.id" "$file")
rm -f "$file"
wait_job "$APP_ID" "$TOKEN_ADMIN" "$DEPLOYMENT_JOB_ID" "deploy_release"

file=$(request_assert_200 POST "/api/repos/${APP_ID}/environments/${ENV_ID}/promote" "$TOKEN_ADMIN" "{\"release_id\":\"${RELEASE_ID}\"}")
PROMOTE_JOB_ID=$(jq -r ".data.job.id" "$file")
PROMOTE_DEPLOYMENT_ID=$(jq -r ".data.deployment.id" "$file")
rm -f "$file"
wait_job "$APP_ID" "$TOKEN_ADMIN" "$PROMOTE_JOB_ID" "promote_release"

file=$(request_assert_200 POST "/api/repos/${APP_ID}/environments/${ENV_ID}/rollback" "$TOKEN_ADMIN" "{\"release_id\":\"${RELEASE_ID}\",\"mode\":\"redeploy_prior_tag\",\"approvals\":[\"${USER_ID}\",\"${REVIEWER_ID}\"]}")
ROLLBACK_JOB_ID=$(jq -r ".data.job.id" "$file")
ROLLBACK_DEPLOYMENT_ID=$(jq -r ".data.deployment.id" "$file")
rm -f "$file"
wait_job "$APP_ID" "$TOKEN_ADMIN" "$ROLLBACK_JOB_ID" "rollback_release"

echo "[9/11] Temp environment lifecycle"
file=$(request_assert_200 POST "/api/repos/${APP_ID}/temp-envs" "$TOKEN_ADMIN" "{\"kind\":\"workspace\",\"source_id\":\"${WORKSPACE_ID}\"}")
...
rm -f "$file"
wait_job "$APP_ID" "$TOKEN_ADMIN" "$TEMP_PROVISION_JOB_ID" "temp_env_provision"
request_assert_200 POST "/api/repos/${APP_ID}/temp-envs/${TEMP_ENV_ID}/extend" "$TOKEN_ADMIN" "{\"seconds\":7200}" >/dev/null
file=$(request_assert_200 DELETE "/api/repos/${APP_ID}/temp-envs/${TEMP_ENV_ID}" "$TOKEN_ADMIN")
TEMP_EXPIRE_JOB_ID=$(jq -r ".data.job.id" "$file")
rm -f "$file"
wait_job "$APP_ID" "$TOKEN_ADMIN" "$TEMP_EXPIRE_JOB_ID" "temp_env_expire"
request_assert_200 POST "/api/repos/${APP_ID}/temp-envs/${TEMP_ENV_ID}/undo-expire" "$TOKEN_ADMIN" >/dev/null

echo "[10/11] Collecting final states"
file=$(request_assert_200 GET "/api/repos/${APP_ID}/jobs?page=1&limit=100" "$TOKEN_ADMIN")
JOBS_FILE="$file"

file=$(request_assert_200 GET "/api/repos/${APP_ID}/changesets/${CHANGESET_ID}" "$TOKEN_ADMIN")
CHANGESET_STATE=$(jq -r ".data.changeset.state" "$file")
rm -f "$file"
file=$(request_assert_200 GET "/api/repos/${APP_ID}/releases/${RELEASE_ID}" "$TOKEN_ADMIN")
RELEASE_STATE=$(jq -r ".data.state" "$file")
rm -f "$file"
file=$(request_assert_200 GET "/api/repos/${APP_ID}/deployments?page=1&limit=50" "$TOKEN_ADMIN")
...
rm -f "$file"
file=$(request_assert_200 GET "/api/repos/${APP_ID}/temp-envs?page=1&limit=50" "$TOKEN_ADMIN")
